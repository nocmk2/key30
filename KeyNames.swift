import Foundation

enum KeyNames {
    static func name(for keyCode: UInt16) -> String {
        let keyNames: [UInt16: String] = [
            // Letter keys
            0x00: "A", 0x01: "S", 0x02: "D", 0x03: "F", 0x04: "H",
            0x05: "G", 0x06: "Z", 0x07: "X", 0x08: "C", 0x09: "V",
            0x0B: "B", 0x0C: "Q", 0x0D: "W", 0x0E: "E", 0x0F: "R",
            0x10: "Y", 0x11: "T", 0x25: "L", 0x26: "J", 0x28: "K",
            0x2D: "N", 0x2E: "M", 0x1F: "O", 0x20: "U", 0x22: "I", 0x23: "P",

            // Number keys
            0x12: "1", 0x13: "2", 0x14: "3", 0x15: "4", 0x17: "5",
            0x16: "6", 0x1A: "7", 0x1C: "8", 0x19: "9", 0x1D: "0",

            // Punctuation
            0x1B: "-", 0x18: "=", 0x21: "[", 0x1E: "]", 0x2A: "\\",
            0x29: ";", 0x27: "'", 0x2B: ",", 0x2F: ".", 0x2C: "/", 0x32: "`",

            // Modifiers
            0x37: "Command", 0x38: "Shift", 0x39: "Caps Lock",
            0x3A: "Option", 0x3B: "Control",
            0x3C: "Right Shift", 0x3D: "Right Option",
            0x3E: "Right Control", 0x36: "Right Command",
            0x3F: "Function",

            // Navigation and editing
            0x31: "Space", 0x24: "Return", 0x4C: "Enter", 0x30: "Tab",
            0x33: "Delete", 0x75: "Forward Delete", 0x35: "Escape",
            0x73: "Home", 0x77: "End", 0x74: "Page Up", 0x79: "Page Down",
            0x7B: "←", 0x7C: "→", 0x7D: "↓", 0x7E: "↑",

            // F keys
            0x7A: "F1", 0x78: "F2", 0x63: "F3", 0x76: "F4",
            0x60: "F5", 0x61: "F6", 0x62: "F7", 0x64: "F8",
            0x65: "F9", 0x6D: "F10", 0x67: "F11", 0x6F: "F12",
            0x69: "F13", 0x6B: "F14", 0x71: "F15", 0x6A: "F16",
            0x40: "F17", 0x4F: "F18", 0x50: "F19", 0x5A: "F20",

            // Keypad
            0x52: "Keypad 0", 0x53: "Keypad 1", 0x54: "Keypad 2", 0x55: "Keypad 3",
            0x56: "Keypad 4", 0x57: "Keypad 5", 0x58: "Keypad 6", 0x59: "Keypad 7",
            0x5B: "Keypad 8", 0x5C: "Keypad 9", 0x45: "Keypad +", 0x4E: "Keypad -",
            0x43: "Keypad *", 0x4B: "Keypad /", 0x41: "Keypad .", 0x51: "Keypad =",
            0x47: "Clear"
        ]

        return keyNames[keyCode] ?? "Key(0x\(String(format: "%02X", keyCode)))"
    }
}
